function [hHist, sHist, vHist] = hsvHistogram(image,numberOfLevelsForH,numberOfLevelsForS,numberOfLevelsForV)
...
h = image(:, :, 1);
hHist = zeros(1, numberOfLevelsForH);
s = image(:, :, 2);
sHist = zeros(1, numberOfLevelsForS);
v = image(:, :, 3);
vHist = zeros(1, numberOfLevelsForV);
v = histeq(v);

hBins = numberOfLevelsForH - 1;
sBins = numberOfLevelsForS - 1;
vBins = numberOfLevelsForV - 1;

% Put all pixels into one of the "numberOfLevels" levels.
for row = 1:size(h, 1)
    for col = 1 : size(h, 2)
        quantizedHValue = ceil(hBins * h(row, col)) +1;
        quantizedSValue = ceil(sBins * s(row, col)) +1;
        quantizedVValue = ceil(vBins * v(row, col)) +1;
        hHist(quantizedHValue) = hHist(quantizedHValue) + 1;
        sHist(quantizedSValue) = sHist(quantizedSValue) + 1;
        vHist(quantizedVValue) = vHist(quantizedVValue) + 1;
    end
end
